...
c           A.C. thetax=dabs(datan(p(1)/p(3))) in ../../Source/kin_functions.f        
c                theta=dacos(p(3)/dsqrt(p(1)**2+p(2)**2+p(3)**2))
            if(
...
c...  Check: were inclusive criteria satisfied?      
      if(has_a_f.and. (.not. xgoodf)) then
