	...
	if (n_inside != 0) {
		...
		case Proc_Nuclelastic: //nuclear elastic (coherent)
			...
			if (sigma == 0) {
				...
				//final state recoil
				if (heprup.procid == Proc_Pelastic) {
					hepeup.IDUP.push_back(92212);
				} else if (heprup.procid == Proc_Eelastic) {
					hepeup.IDUP.push_back(911);
				} else if (heprup.procid == Proc_Nuclelastic) {
					hepeup.IDUP.push_back(999);
				}
				hepeup.ISTUP.push_back(1);
				hepeup.MOTHUP.push_back(std::make_pair(particle + 1, particle + 1));
				hepeup.ICOLUP.push_back(std::make_pair(0, 0));
...
int main(int argc, char *argv[]) {
	...
	if (argc != 3) {
		cout << " Usage: " << appName << " input_file" << " output_file_name" << endl;
	...
	//nuclear part: setting variables
	m_utils->setMnucl(inputReader->heprup.NUCLMASS);
	m_utils->setZnucl(inputReader->heprup.NUCLZ);
	m_utils->setThrnucl(inputReader->heprup.nuclTHR);

	m_utils->PrintParameters();
	m_utils->Setup();
