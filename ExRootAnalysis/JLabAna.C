...
void JLabAna(string fname){
  ...
  //  chain->SetProof();
	...
	TRootLHEFEvent* evt;
	TRootLHEFParticle *particle,*chi,*achi,*e;
	
	TLorentzVector Pchi,Pachi;
	TLorentzVector *Pchi_in;
	TLorentzVector Pe; //final state e- for elastic e- scattering
	...
	TVector3 vin,vout,vhit;
        ...
        //Histograms: chi
	TH1D *hEnergy0=new TH1D("hEnergy0","hEnergy0; #chi Energy (GeV)",100,0,11);
	TH1D *hEnergy1=new TH1D("hEnergy1","hEnergy1",100,0,11);
	...
	TH2D *hLvsE0=new TH2D("hLvsE","hLvsE; E(GeV); L(m)",100,0.,11,100,0,5.);
	
	TH2D *hLvsTheta0=new TH2D("hLvsTheta0","hLvsTheta0; Theta(deg); L(m)",100,0.,20.,100,0,5.);
	
	//Histograms: e-
	TH1D *hEnergyElectron0=new TH1D("hEnergyElectron0","hEnergyElectron0",100,0,11);
	...
	for (int ii=0;ii<N;ii++){
		...
		if (e!=NULL){
		  Pe.SetXYZT(e->Px,e->Py,e->Pz,e->E);



		  hEnergyElectron0->Fill(Pe.E());
		}
