...
bool ExRootLHEFReader::ReadBlock(ExRootTreeBranch *branch)
{
  ...
  //read last lines
    ...
    if ((strcmp(token[0],"#")==0)&&(strcmp(token[1],"IN:")==0)){
      fVin.SetXYZ(atof(token[2]),atof(token[3]),atof(token[4]));
    }
    else if ((strcmp(token[0],"#")==0)&&(strcmp(token[1],"OUT:")==0)){
      fVout.SetXYZ(atof(token[2]),atof(token[3]),atof(token[4]));
    }
    else if ((strcmp(token[0],"#")==0)&&(strcmp(token[1],"HIT:")==0)){
      fVhit.SetXYZ(atof(token[2]),atof(token[3]),atof(token[4]));
    }
    else if ((strcmp(token[0],"#")==0)&&(strcmp(token[1],"L:")==0)){
     fL=atof(token[2]);
    }
    else if ((strcmp(token[0],"#")==0)&&(strcmp(token[1],"W:")==0)){
      fW=atof(token[2]);
      --fEventCounter;
    }
    delete[] token;
...
void ExRootLHEFReader::AnalyzeEvent(ExRootTreeBranch *branch, long long eventNumber)
{
  ...
  element->Vin.SetXYZ(fVin.X(),fVin.Y(),fVin.Z());
  element->Vout.SetXYZ(fVout.X(),fVout.Y(),fVout.Z());
  element->Vhit.SetXYZ(fVhit.X(),fVhit.Y(),fVhit.Z());
