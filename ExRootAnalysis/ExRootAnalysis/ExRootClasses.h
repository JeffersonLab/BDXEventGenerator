...
public:
  ...
  //A.C. Here I add what I need for BDX
  TVector3 Vin; //where the chi enters the fiducial volume (meters)
  TVector3 Vout; //where the chi would exit the fiducial volume (meters)
  TVector3 Vhit; //the interaction point (meters)
  double L; //the effective fiducial volume length (cm)
  double W; //the event weight per EOT. To have the number of scattering events per EOT, sum all these weights in the file
